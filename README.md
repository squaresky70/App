...
| 바탕화면 위젯 | 창 맨 위 "캘린더" 옆 **위젯 추가** 를 누르면 반투명 유리 달력 위젯이 뜬다. 음력·공휴일·일정이 보이고, 다른 창을 누르면 바탕화면 쪽으로 가라앉는다. 머리 부분을 끌어 옮기고 가장자리로 크기 조절, 날짜 **더블클릭** 시 앱이 그 날짜로 열린다. 본 창을 닫아도 위젯은 남고, 켜 둔 채 끝내면 다음 실행 때 같은 자리에 다시 뜬다 |

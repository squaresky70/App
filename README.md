...
| 바탕화면 위젯 | 창 맨 위 "캘린더" 옆 **위젯 추가** 를 누르면 반투명 유리 달력 위젯이 뜬다. 음력·공휴일·일정이 보이고, 다른 창을 누르면 바탕화면 쪽으로 가라앉는다. 머리 부분을 끌어 옮기고 가장자리로 크기 조절, 날짜 **더블클릭** 시 앱이 그 날짜로 열린다. 오른쪽 아래 **투명도** 슬라이더(0~100%)로 배경 진하기를 조절한다(가장 투명해도 글자가 읽히도록 옅은 색은 남는다). 본 창을 닫아도 위젯은 남고, 켜 둔 채 끝내면 다음 실행 때 같은 자리에 다시 뜬다. 위젯을 켜면 **시작프로그램에 등록**되어 컴퓨터를 다시 켜도 위젯만 저절로 뜬다(위젯을 닫으면 등록도 해제) |
...
위젯을 켰는지, 그 위치·크기·투명도는 같은 폴더의 `settings.json` 에 기억합니다.

위젯 자동 실행은 아래 레지스트리 값으로 등록됩니다(관리자 권한 불필요, 위젯을 닫으면 지워짐).

```
HKCU\Software\Microsoft\Windows\CurrentVersion\Run
  NotionCalendarWidget = "…\NotionCalendar.exe" --widget
```
